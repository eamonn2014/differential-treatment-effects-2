                ...
                sidebarLayout(
                    ...
                    sidebarPanel( width=3 ,
                                  ...
                                  div(
                                      ...
                                      div(h5(tags$span(style="color:blue", "Enter predictor coefficients:"))),
                                      ...
                                      splitLayout(
                                          textInput("v1m", div(h5(tags$span(style="color:blue", "Intercept"))), value= "log(.5)"),
                                          textInput("v2m", div(h5(tags$span(style="color:blue", "treatment"))), value= "1"),
                                          textInput("v3m", div(h5(tags$span(style="color:blue", "Sex"))), value= "log(1)")
                                          
                                      ),
                                      
                                      textInput('backm', 
                                                div(h5(tags$span(style="color:blue",
                                                                 "Back (factor), effects for n-1 levels"))),  
                                                "log(.95), log(0.9), log(0.9), log(0.85), log(0.85), log(0.8), log(0.8)"),
                                      ...
                                      splitLayout(
                                          ...
                                          textInput("v5m", div(h5(tags$span(style="color:blue", "age"))), value= "-.1/(65-18)"),
                                          textInput("v6m", div(h5(tags$span(style="color:blue", "berlin.spine.effects"))), value= "log(1.1)")
                                          
                                      ),
                                      
                                      
                                      splitLayout(
                                          textInput("v7m", div(h5(tags$span(style="color:blue", "berlin.spj.effects"))), value= "log(1.1)"),
                                          textInput("v8m", div(h5(tags$span(style="color:blue", "deg.chg.effects"))), value= "-1/(40)"),
                                          textInput("v9m", div(h5(tags$span(style="color:blue", "FASSS"))), value= "1/51")
                                          
                                      ),
                                      
                                      
                                      splitLayout(
                                          textInput("v10m", div(h5(tags$span(style="color:blue", "disease duration"))), value= "1/(75)"),
                                          textInput("v11m", div(h5(tags$span(style="color:blue", "disease severity"))), value= "1/20") #,
                                         # textInput("v12m", div(h5(tags$span(style="color:blue", "Sex (binary)"))), value= "log(0.5)")
                                          
                                      ),
                                      
                                      div(h5(tags$span(style="color:blue", "Enter interaction coefficients:"))),
                                    ...
                                    #  disease.duration.effects      <- 1/(75)  # we expect unit change effect of 1.028984
                                     # disease.severity.effects      <- 1/(20)  # we  
                                      ##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# interaction effects
                                      textInput('backi', 
                                                div(h5(tags$span(style="color:blue",
                                                    "Back interaction (factor), effects for n-1 levels"))),  "0, log(1.1), log(1.2),log(1.4), 0 , 0, 0"),
                                    
                                      textInput('smoki', 
                                                div(h5(tags$span(style="color:blue",
                                                                        "Smoking interaction (factor), effects for n-1 levels"))),  "log(.8), log(0.6) "),
                                    
                                      textInput('grp1', 
                                                div(h5(tags$span(style="color:blue",
                                                    "Sex, age, berlin spine, berline spj interaction effects"))),  "log(1), log(1), log(1),log(1)"),
                                    
                                      textInput('grp2', 
                                                div(h5(tags$span(style="color:blue",
                                                    "degenerative changes, FASSS, disease duration, disease severity interaction effects"))),  "- 1/(10), -1/(51), -log(1.1),-log(1.1)"),
                   ...
                   ")),
                                  ...
                                  tabPanel("11 Contrast example", value=3, 
                                          ...
                                          # textInput('levz', 
                                           #          strong(div(h5(tags$span(style="color:blue", "Contrast to compare smoking levels")))), c("smoker","Never")),
...
server <- shinyServer(function(input, output   ) {
    ...
    #starting new, the inputs are read in
    random.samplex <- reactive({
        ...
        # main effects
        # writing like this I can write log and fraction into input boxes!
        v1m <- as.numeric(    eval(parse(text= (input$v1m)) ) )
        v2m <- as.numeric(    eval(parse(text= (input$v2m)) ) )
        v3m <- as.numeric(    eval(parse(text= (input$v3m)) ) )    
       # v4m <- as.numeric(    eval(parse(text= (input$v4m)) ) )   
        
        v4m <- as.numeric(unlist(strsplit(input$v4m,",")))
        v4m1=v4m[1]
        v4m2=v4m[2]
        
        v5m <- as.numeric(    eval(parse(text= (input$v5m)) ) )  
        v6m <- as.numeric(    eval(parse(text= (input$v6m)) ) ) 
        v7m <- as.numeric(    eval(parse(text= (input$v7m)) ) )
        v8m <- as.numeric(    eval(parse(text= (input$v8m)) ) )
        v9m <- as.numeric(    eval(parse(text= (input$v9m)) ) )
        v10m <- as.numeric(    eval(parse(text= (input$v10m)) ) )
        v11m <- as.numeric(    eval(parse(text= (input$v11m)) ) )
        v12m <- as.numeric(    eval(parse(text= (input$v12m)) ) )

        backm <- as.numeric(unlist(strsplit(input$backm,",")))
        ...
        backi <- as.numeric(unlist(strsplit(input$backi,",")))
        ...
        smoki <- as.numeric(unlist(strsplit(input$smoki,",")))
        
        s1i=smoki[1]
        s2i=smoki[2]  
        
        grp1 <- as.numeric(unlist(strsplit(input$grp1,",")))
        ...
        grp2 <- as.numeric(unlist(strsplit(input$grp2,",")))
        ...
        return(list(
            v1m=v1m, v2m=v2m, v3m=v3m, v4m1=v4m1, v4m2=v4m2, vm5=v5m, v6m=v6m, v7m=v7m, v8m=v8m, v9m=v9m, v10m=v10m, v11m=v11m, v12m=v12m ,
    ...
    designx <- reactive({
        ...
        # transfer effects to objects with sensible names

        intercept <-            v1m # log(.5) # 50% prob of response
        trt.effects <-          v2m #                # we expect exp(1) OR that is OR= 2.7183, large effect
        sex.effects <-          v3m # log(1)           # we expect no effect, OR=1
        back.effects <-         c(b1,b2,b3,b4,b5,b6,b7)  # we expect coefficients to get lower compared to ref level
        smoking.effects <-      c(v4m1, v4m2)
        
        age.effects <-          v5m        # we expect unit change effect of .99
        berlin.spine.effects <- v6m          # no spine issue do better OR 1.1
        berlin.spj.effects <-   v7m         # no spine issue do better OR 1.1
        deg.chg.effects <-      v8m           # we expect OR of exp(-0.05) , OR 1.105
        FASSS.effects <-        v9m           # we expect unit change effect of  1.0198
        disease.duration.effects      <- v10m # we expect unit change effect of 1.028984
        disease.severity.effects      <- v11m  # we expect unit change effect of 1.051271
        
        # interaction effects
        sex.int.effects <-     sexi            # truth no difference depending on sex
        back.int.effects <-  c(b1i,b2i,b3i,b4i,b5i,b6i,b7i) 
        
        smoking.int.effects <- c(s1i,s2i)  # quitters do worse than never, smokers do worse again
        age.int.effects <-     agei            # we expect no int effect, OR=1 
        berlin.spine.effects <-bspine           # we expect no int effect, OR=1 
        berlin.spj.effects <-  bspj             # we expect no int effect, OR=1 
        deg.chg.int.effects <- dchgi             #  we expect OR of exp(0.1) per unit change
        FASSS.int.effects <-   fasssi           # we expect unit change effect of  1.0198
        
        disease.duration.int.effects      <- ddur  # we expect  OR=.9
        disease.severity.int.effects      <- dsev  # we expect  OR=.9
        ...
        #x<-rexp(51)
        x<- exp(-.045*1:457)
        FASSS      <- sample(0:456, N, prob = (sort(x, decreasing =T)/sum(x)), replace=TRUE) 
        
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        x<- exp(-.45*1:7)
        deg.chg       <- sample(0:6, N, prob = (sort(x, decreasing =T)/sum(x)), replace=TRUE) 
        ...
        return(list(    
            ...
            sex.effects =              sex.effects     ,       
            back.effects =             back.effects      ,   
            smoking.effects =          smoking.effects,
            age.effects =              age.effects        ,
            berlin.spine.effects =     berlin.spine.effects  ,        
            berlin.spj.effects =       berlin.spj.effects      ,    
            deg.chg.effects =          deg.chg.effects        ,
            FASSS.effects =            FASSS.effects         ,
            disease.duration.effects = disease.duration.effects ,
            disease.severity.effects = disease.severity.effects ,
            
            # interaction effects
            sex.int.effects  =        sex.int.effects       ,    
            back.int.effects  =       back.int.effects ,
            smoking.int.effects =     smoking.int.effects , 
            age.int.effects =         age.int.effects         ,   
            berlin.spine.effects =    berlin.spine.effects  ,          
            berlin.spj.effects =      berlin.spj.effects   ,           
            deg.chg.int.effects =     deg.chg.int.effects   ,         
            FASSS.int.effects =       FASSS.int.effects       ,    
            disease.duration.int.effects     = disease.duration.int.effects,
            disease.severity.int.effects     =disease.severity.int.effects ,
            ...
            FASSS  =             FASSS ,
            deg.chg      =       deg.chg   ,
            berlin.spine=        berlin.spine,
            berlin.spj =         berlin.spj,
            trt=                 trt,
            sex=                 sex,
            smoking=             smoking,
            back=                back,
    ...
    lp1x <- reactive({
        ...
        trt.effects  =             d$trt.effects
        sex.effects =              d$sex.effects     
        back.effects =             d$back.effects     
        smoking.effects =          d$smoking.effects
        age.effects =              d$age.effects        
        berlin.spine.effects =     d$berlin.spine.effects        
        berlin.spj.effects =       d$berlin.spj.effects        
        deg.chg.effects =          d$deg.chg.effects        
        FASSS.effects =            d$FASSS.effects         
        disease.duration.effects = d$disease.duration.effects 
        disease.severity.effects = d$disease.severity.effects 
        
        # interaction effects
        sex.int.effects  =        d$sex.int.effects         
        back.int.effects  =       d$back.int.effects 
        smoking.int.effects =     d$smoking.int.effects 
        age.int.effects =         d$age.int.effects          
        berlin.spine.effects =    d$berlin.spine.effects          
        berlin.spj.effects =      d$berlin.spj.effects             
        deg.chg.int.effects =     d$deg.chg.int.effects          
        FASSS.int.effects =       d$FASSS.int.effects       
        disease.duration.int.effects     = d$disease.duration.int.effects
        disease.severity.int.effects     = d$disease.severity.int.effects 
        ...
        FASSS  =             d$FASSS 
        deg.chg      =       d$deg.chg   
        berlin.spine=        d$berlin.spine
        berlin.spj =         d$berlin.spj
        trt=                 d$trt
        sex=                 d$sex
        smoking=             d$smoking
        back=                d$back
        disease.duration  =  d$disease.duration
        disease.severity =   d$disease.severity   
        ...
        if ( (input$Design) == "Treatment interacts with all variables" )  {
            ...
            lp <- as.numeric(as.matrix(X) %*% as.matrix(all.effects)) # 
            ...
            lp <- as.numeric(as.matrix(X1) %*% as.matrix(all.effects2)) # 
            
        }
        
        y <- ifelse(randomi < plogis(lp), 1, 0)   # one liner RANDOM!!!
        
        dat <- cbind.data.frame(y,trt ,sex,back,smoking,age,FASSS,deg.chg,berlin.spine,berlin.spj, disease.duration ,disease.severity)  
    ...
    # disease.severity
      ...
      C <- lrm(y~  trt +  sex+ back + smoking + age + berlin.spine + berlin.spj + deg.chg + FASSS + disease.duration + disease.severity,  da, y=TRUE, x=TRUE)  # main
        ...
        da$trt <- relevel(da$trt, "xdrug")
        Aref2 <- lrm(y~  trt * (sex+ back + smoking + age + berlin.spine + berlin.spj + deg.chg + FASSS + disease.duration + disease.severity), da, y=TRUE ,x=TRUE)  # all interact with trt
         
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        return(list(  A=A, C=C, f=f, outputx=outputx, Aref2 = Aref2)) 
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    rexvx <- reactive({
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        return(list(   L2= L2,   AICA = AICA , AICC = AICC)) 
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    contrastzx<- reactive({
       ...
       # B<-  X$B  # trt x smoke
        ...
        k1 <- contrast(A, list(smoking=c("current"), trt=c("placebo":"drugX")), list(smoking=c("never"), trt=c("placebo":"drugX")) , fun=exp) 
        z1 <- print(k1, X=TRUE)   
    ...
    cont2x <- reactive({
        ...
        k99 <- contrast(A, list(smoking=M, trt=c("placebo":"drugX")), list(smoking=N, trt=c("placebo":"drugX")) , fun=exp)  # move fun=exp to print
        z99 <- print(k99, X=TRUE)
    ...
    #     return(print(lrtestx()$L1, digits=3))
    # })
    output$L1bx <- renderPrint({
        return(print(lrtestxx()$L2, digits=3))
    })
    # output$L1c <- renderPrint({
    #     return(print(lrtestx()$L3, digits=3))
    # })
